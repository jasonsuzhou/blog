...
function appendPaginator(curPage, perPage, totalCount) {
	...
	var pager = totalCount + " Records Page:" + curPage + "/" + totalPage + 
				...
				"<a href='javascript:queryMessage("+totalPage+")'>Last</a>";
	$('#paginator').append(pager);
	
}
function queryMessage(curPage) {
	$.ajax({
		url : 'adminQueryMessage.do',
		type : 'POST',
...
function gotoEdit(id) {
	self.location.href = "gotoEditPage.do?id="+id;
}
	$(function() {
		queryMessage(1);
	...
	function del(id) {
		...
		if (confirm("Are you sure to delete？")) {
			window.location.href = "adminDeleteMessage.do?id="+id;
		}
